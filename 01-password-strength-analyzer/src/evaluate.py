...
def write_findings(summary: dict, exp_a: dict) -> None:
    ...
    lines += [
        ...
        f"{f['n_good_generated']} CSPRNG-generated passwords (known-good, guess space "
        ...
        "not be believed: the generated controls average 25 characters and the breached "
        f"passwords {f2['mean_len_bad']}, so any length-sensitive score wins by default. "
        "Removing the confound settles it - each control is regenerated by a CSPRNG at "
        "exactly the length of a real breached password, carrying all four classes, so "
        ...
        "",
        "For CSPRNG-generated passwords the true guess space is arithmetic, so the "
